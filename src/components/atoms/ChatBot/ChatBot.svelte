    ...
    {#if isChatbotOpen}
        ...
        <div class="chatbot__block w-[100%] ss:w-[335px] h-[420px] flex flex-col items-start shrink-0 bg-white/10 backdrop-blur-md" in:fly={chatbotBlockIn} out:fly={chatbotBlockOut}>
            ...
            <div class="flex py-4 px-5 gap-4 self-stretch border-t border-solid border-black h-fit box-border">
                <!-- text area -->
                <textarea bind:this={userInput} class="flex self-stretch resize-none flex-[1_0_0] bg-transparent border-none outline-0 h-4 max-h-14 text-xs" placeholder="Type your message..." bind:value={userInputValue} on:input={resizeTextarea}/>
                <!-- send button -->
                <button bind:this={sendButton} class="disabled:opacity-50" on:click={sendQuestion}>
